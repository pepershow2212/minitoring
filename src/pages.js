...
export function joinPage(result) {
  ...
  const status = data.ok
    ? "Открываем Steam…"
    : data.reason === "empty"
      ? "Сейчас никого нет. Steam-лобби появляется только когда кто-то уже в игре. Первый заходит из списка серверов Wardogs."
    ...
    async function poll() {
      ...
      try {
        ...
        statusEl.textContent = data.reason === "empty"
          ? "Сервер пустой. Первый заходит из списка серверов в игре."
          : "Ищем лобби, страница сама кинет в игру.";
      } catch (error) {
